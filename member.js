...
function updateUniqueIdPreview() {
  ...
  } else { display.innerText = "尚未產生"; display.classList.replace('text-blue-700', 'text-slate-700'); }
}
function fillMemberForm(profile) {
  document.getElementById('userName').value = profile.name; document.getElementById('userGender').value = profile.gender || "男";
  document.getElementById('userCohort').value = profile.cohort; document.getElementById('userPhone').value = profile.phone;
  document.getElementById('userStudentId').value = profile.studentId || ""; document.getElementById('userEmail').value = profile.email || "";
  document.getElementById('userAddress').value = profile.address || ""; document.getElementById('userIndustry').value = profile.industry || "";
  document.getElementById('userCompany').value = profile.company || ""; document.getElementById('userJobTitle').value = profile.jobTitle || "";
}
function renderMemberCard(profile) {
  document.getElementById('cardName').innerText = `${profile.name} (${profile.gender || "男"})`; document.getElementById('cardUniqueId').innerText = profile.uniqueId || "尚未產生";
  document.getElementById('displayRole').innerText = profile.role; document.getElementById('cardPhone').innerText = maskPhone(profile.phone);
  document.getElementById('cardStudentId').innerText = profile.studentId || "未提供"; document.getElementById('cardAddress').innerText = maskAddress(profile.address);
}
async function saveBasicProfile() {
  const name = document.getElementById('userName').value; const cohort = document.getElementById('userCohort').value; const phone = document.getElementById('userPhone').value;
  if (!name || !cohort || !phone) { alert("請填寫姓名、畢業屆數與聯絡電話。"); return; }
  const btn = document.getElementById('saveBasicBtn'); btn.disabled = true; btn.innerText = "儲存中...";
  try {
    ...
    if (result.status === 'success') { alert("✅ 基本資料儲存成功！"); checkUserData(currentUserLineId); } else throw new Error(result.message);
...
async function saveDetailProfile() {
  ...
  const btn = document.getElementById('saveDetailBtn'); btn.disabled = true; btn.innerText = "儲存中...";
  try {
    ...
    const result = await response.json();
    if (result.status === 'success') { alert("✅ 進階資料已寫入！"); checkUserData(currentUserLineId); } else throw new Error(result.message);
